...
void cairox_paint_curved_arrow(cairo_t *cr, ArrowHeadType head, CairoxPoint *coordinates, int points, double width, double zoom)
{
    ...
    if (points <= 2) {
        ...
        while (++i < points-1) {
            ...
            // Limit curves to max radius of 25 pixels:
            double dw = MIN(25, MIN(w1, w2) / 2.0); 
            CairoxPoint point_a;
//            CairoxPoint point_b;
            CairoxPoint centre;
...
void cairox_select_colour(cairo_t *cr, int i, double alpha)
{
    if (i < 0) {        /* White */
        ...
        cairo_set_source_rgba(cr, 0.5, 0.0, 0.0, alpha);
    }
    else if (i == 6) {  /* Purple */
        cairo_set_source_rgba(cr, 0.5, 0.0, 1.0, alpha);
    }
    else if (i == 7) {  /* Orange */
        cairo_set_source_rgba(cr, 1.0, 0.5, 0.0, alpha);
    }
...
void cairox_select_colour_scale(cairo_t *cr, double d)
{
    ...
    if (d < 0.0) {
        hsl_to_rgb(0.0 * 0.667, 1.0, 0.5, &r, &g, &b);
    }
    else if (d > 1.0) {
        hsl_to_rgb(1.0 * 0.667, 1.0, 0.5, &r, &g, &b);
    }
    else {
